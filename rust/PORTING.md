...
2. **Topology invariants and operation history.** Generic B-rep validation
   ...
   * **M4** propagates attributes through every operation by declared
     ...
     OCCT has none at kernel level). Accepted at `979cf939`.
   * **M5** stores enclosures on entities. Implemented: a certified bound
     on every vertex, fin and face; `enclosure_missing`,
     `enclosure_exceeds_resolution`, `enclosure_unsound` and
     `uncertified_enclosure` in `Topology::check`; measured bounds in every
     builder, carried through every operation so none falls below an
     input's; OCCT tolerances imported as `Imported` bounds. Every
     comparison against the linear tolerance is now certified or recorded as
