...
| Retained upstream evidence | Every upstream assertion labelled model-independent, mapped-and-verified or lost; mappings verified natively per case; the aggregate tracked, never a raw pass percentage | Bridge reports pass, unsupported, missing and known-failure only; `nbshapes` synthesizes OCCT counts and the history and B-rep bridges verify structure-only seams by count synthesis on every case (T1); the ledger, `.brep` round trips and the native selector are T2 of `TOPOLOGY_MODEL.md` |
