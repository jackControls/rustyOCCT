...
### Extra edge in the oblique triangle-edge case: 3 cases

`triangle_edge_line` with frame 2 returns two edges in **both** native outputs,
at scales 1/8, 1 and 32. At scale 1 the triangle vertices are `(2,-3,5)`,
`(10,1,5)`, `(2,5,9)`; the line runs through `(-2,-5,5)` and `(14,3,5)`.
The true intersection is the first triangle edge, from `(2,-3,5)` to `(10,1,5)`.
The additional native edge ends at `(2,5,9)`, which cannot lie on a line whose
z coordinate is identically 5. The error is far outside the unchanged budget.
