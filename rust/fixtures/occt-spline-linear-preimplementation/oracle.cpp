// Native observations before implementing exact spline/linear intersections.
...
int main() {
  ...
  while (std::getline(std::cin,row)) {
    ...
    try {
      ...
      if (kind=="L") makeLine=BRepBuilderAPI_MakeEdge(line);
      else makeLine=BRepBuilderAPI_MakeEdge(line,0.,length);
      if (!makeLine.IsDone()) {
