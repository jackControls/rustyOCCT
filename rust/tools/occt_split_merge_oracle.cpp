...
void report(std::ostream& out, const std::string& stage, const std::vector<std::pair<std::string, TopoDS_Shape>>& args,
  ...
  // Structure-only shapes of the arguments and of the result.
  ShapeMap structure = structure_only(result);
  for (const auto& [name, arg] : args) {
    ShapeMap s = structure_only(arg);
    for (int i = 1; i <= s.Extent(); ++i) structure.Add(s(i));
  }
  for (int i = 1; i <= structure.Extent(); ++i) out << "S " << stage << ' ' << signature(structure(i)) << '\n';
  int k = 0;
